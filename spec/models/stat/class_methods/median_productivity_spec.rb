RSpec.describe Stat, '.median_productivity' do
  ...
  it 'returns 1 hour when the only stat is today' do
    Stat.create!(user: user, value: 35.minutes, timestamp: Time.zone.now)
  ...
  it 'returns the median of the stats for past 2 weeks, excluding today' do
    Stat.create!(user: user, value: 35.minutes, timestamp: 1.month.ago)
    Stat.create!(user: user, value: 35.minutes, timestamp: Time.zone.now)
    Stat.create!(user: user, value: 35.minutes, timestamp: 1.day.ago)
    ...
    Stat.create!(user: user, value: 1.hour, timestamp: 2.days.ago)
    result = Stat.median_productivity(median_class: mock_median)
    ...
    Stat.create!(user: user, value: 45.minutes, timestamp: 3.days.ago)
    result = Stat.median_productivity(median_class: mock_median)
    ...
    Stat.create!(user: user, value: 25.minutes, timestamp: 4.days.ago)
    result = Stat.median_productivity(median_class: mock_median)
